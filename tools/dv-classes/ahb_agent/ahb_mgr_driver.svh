...
task ahb_mgr_driver::set_data_phase_signals();
  if (m_cur_data_phase == null) begin
    ...
    if (m_cur_data_phase.m_write && |(m_cur_data_phase.m_wdata >> m_vif.data_width)) begin
      `uvm_error(get_full_name(),
                 $sformatf("Cannot represent m_wdata = 0x%0h. The interface DATA_WIDTH is %0d.",
                           m_cur_data_phase.m_wdata, m_vif.data_width))
    end

    if (m_cur_data_phase.m_write &&
        |(m_cur_data_phase.m_wstrb >> ((m_vif.data_width + 7) / 8))) begin
      `uvm_error(get_full_name(),
                 $sformatf({"Cannot represent m_strb = 0x%0h. ",
                            "The interface DATA_WIDTH is %0d, giving %0d strobe bits."},
                           m_cur_data_phase.m_wstrb, m_vif.data_width, (m_vif.data_width + 7) / 8))
    end
