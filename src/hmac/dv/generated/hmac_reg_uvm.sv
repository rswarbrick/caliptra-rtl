// This file was autogenerated by PeakRDL-uvm
package hmac_reg_uvm;
    `include "uvm_macros.svh"
    import uvm_pkg::*;
    import dv_base_reg_pkg::*;
    `include "hmac_reg_covergroups.svh"
    // reg - hmac_reg::HMAC512_NAME
    class hmac_reg__HMAC512_NAME extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_NAME)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_NAME_bit_cg NAME_bit_cg[32];
        hmac_reg__HMAC512_NAME_fld_cg fld_cg;
        rand dv_base_reg_field NAME;

        function new(string name = "hmac_reg__HMAC512_NAME");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.NAME = dv_base_reg_field::type_id::create("NAME");
            this.NAME.configure(this, 32, 0, "RO", 1, 'h0, 0, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(NAME_bit_cg[bt]) NAME_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_NAME

    // reg - hmac_reg::HMAC512_VERSION
    class hmac_reg__HMAC512_VERSION extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_VERSION)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_VERSION_bit_cg VERSION_bit_cg[32];
        hmac_reg__HMAC512_VERSION_fld_cg fld_cg;
        rand dv_base_reg_field VERSION;

        function new(string name = "hmac_reg__HMAC512_VERSION");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.VERSION = dv_base_reg_field::type_id::create("VERSION");
            this.VERSION.configure(this, 32, 0, "RO", 1, 'h0, 0, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(VERSION_bit_cg[bt]) VERSION_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_VERSION

    // reg - hmac_reg::HMAC512_CTRL
    class hmac_reg__HMAC512_CTRL extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_CTRL)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_CTRL_bit_cg INIT_bit_cg[1];
        hmac_reg__HMAC512_CTRL_bit_cg NEXT_bit_cg[1];
        hmac_reg__HMAC512_CTRL_bit_cg ZEROIZE_bit_cg[1];
        hmac_reg__HMAC512_CTRL_bit_cg MODE_bit_cg[1];
        hmac_reg__HMAC512_CTRL_bit_cg CSR_MODE_bit_cg[1];
        hmac_reg__HMAC512_CTRL_bit_cg LAST_bit_cg[1];
        hmac_reg__HMAC512_CTRL_bit_cg Reserved_bit_cg[1];
        hmac_reg__HMAC512_CTRL_fld_cg fld_cg;
        rand dv_base_reg_field INIT;
        rand dv_base_reg_field NEXT;
        rand dv_base_reg_field ZEROIZE;
        rand dv_base_reg_field MODE;
        rand dv_base_reg_field CSR_MODE;
        rand dv_base_reg_field LAST;
        rand dv_base_reg_field Reserved;

        function new(string name = "hmac_reg__HMAC512_CTRL");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.INIT = dv_base_reg_field::type_id::create("INIT");
            this.INIT.configure(this, 1, 0, "WO", 1, 'h0, 1, 1, 0);
            this.NEXT = dv_base_reg_field::type_id::create("NEXT");
            this.NEXT.configure(this, 1, 1, "WO", 1, 'h0, 1, 1, 0);
            this.ZEROIZE = dv_base_reg_field::type_id::create("ZEROIZE");
            this.ZEROIZE.configure(this, 1, 2, "WO", 1, 'h0, 1, 1, 0);
            this.MODE = dv_base_reg_field::type_id::create("MODE");
            this.MODE.configure(this, 1, 3, "WO", 0, 'h1, 1, 1, 0);
            this.CSR_MODE = dv_base_reg_field::type_id::create("CSR_MODE");
            this.CSR_MODE.configure(this, 1, 4, "WO", 0, 'h0, 1, 1, 0);
            this.LAST = dv_base_reg_field::type_id::create("LAST");
            this.LAST.configure(this, 1, 5, "WO", 1, 'h0, 1, 1, 0);
            this.Reserved = dv_base_reg_field::type_id::create("Reserved");
            this.Reserved.configure(this, 1, 6, "WO", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(INIT_bit_cg[bt]) INIT_bit_cg[bt] = new();
                foreach(NEXT_bit_cg[bt]) NEXT_bit_cg[bt] = new();
                foreach(ZEROIZE_bit_cg[bt]) ZEROIZE_bit_cg[bt] = new();
                foreach(MODE_bit_cg[bt]) MODE_bit_cg[bt] = new();
                foreach(CSR_MODE_bit_cg[bt]) CSR_MODE_bit_cg[bt] = new();
                foreach(LAST_bit_cg[bt]) LAST_bit_cg[bt] = new();
                foreach(Reserved_bit_cg[bt]) Reserved_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_CTRL

    // reg - hmac_reg::HMAC512_STATUS
    class hmac_reg__HMAC512_STATUS extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_STATUS)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_STATUS_bit_cg READY_bit_cg[1];
        hmac_reg__HMAC512_STATUS_bit_cg VALID_bit_cg[1];
        hmac_reg__HMAC512_STATUS_fld_cg fld_cg;
        rand dv_base_reg_field READY;
        rand dv_base_reg_field VALID;

        function new(string name = "hmac_reg__HMAC512_STATUS");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.READY = dv_base_reg_field::type_id::create("READY");
            this.READY.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            this.VALID = dv_base_reg_field::type_id::create("VALID");
            this.VALID.configure(this, 1, 1, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(READY_bit_cg[bt]) READY_bit_cg[bt] = new();
                foreach(VALID_bit_cg[bt]) VALID_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_STATUS

    // reg - hmac_reg::HMAC512_KEY
    class hmac_reg__HMAC512_KEY extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_KEY)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_KEY_bit_cg KEY_bit_cg[32];
        hmac_reg__HMAC512_KEY_fld_cg fld_cg;
        rand dv_base_reg_field KEY;

        function new(string name = "hmac_reg__HMAC512_KEY");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.KEY = dv_base_reg_field::type_id::create("KEY");
            this.KEY.configure(this, 32, 0, "WO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(KEY_bit_cg[bt]) KEY_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_KEY

    // reg - hmac_reg::HMAC512_BLOCK
    class hmac_reg__HMAC512_BLOCK extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_BLOCK)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_BLOCK_bit_cg BLOCK_bit_cg[32];
        hmac_reg__HMAC512_BLOCK_fld_cg fld_cg;
        rand dv_base_reg_field BLOCK;

        function new(string name = "hmac_reg__HMAC512_BLOCK");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.BLOCK = dv_base_reg_field::type_id::create("BLOCK");
            this.BLOCK.configure(this, 32, 0, "WO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(BLOCK_bit_cg[bt]) BLOCK_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_BLOCK

    // reg - hmac_reg::HMAC512_TAG
    class hmac_reg__HMAC512_TAG extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_TAG)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_TAG_bit_cg TAG_bit_cg[32];
        hmac_reg__HMAC512_TAG_fld_cg fld_cg;
        rand dv_base_reg_field TAG;

        function new(string name = "hmac_reg__HMAC512_TAG");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.TAG = dv_base_reg_field::type_id::create("TAG");
            this.TAG.configure(this, 32, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(TAG_bit_cg[bt]) TAG_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_TAG

    // reg - hmac_reg::HMAC512_LFSR_SEED
    class hmac_reg__HMAC512_LFSR_SEED extends dv_base_reg;
        `uvm_object_utils(hmac_reg__HMAC512_LFSR_SEED)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__HMAC512_LFSR_SEED_bit_cg LFSR_SEED_bit_cg[32];
        hmac_reg__HMAC512_LFSR_SEED_fld_cg fld_cg;
        rand dv_base_reg_field LFSR_SEED;

        function new(string name = "hmac_reg__HMAC512_LFSR_SEED");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.LFSR_SEED = dv_base_reg_field::type_id::create("LFSR_SEED");
            this.LFSR_SEED.configure(this, 32, 0, "WO", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(LFSR_SEED_bit_cg[bt]) LFSR_SEED_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__HMAC512_LFSR_SEED

    // reg - kv_read_ctrl_reg
    class kv_read_ctrl_reg extends dv_base_reg;
        `uvm_object_utils(kv_read_ctrl_reg)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        kv_read_ctrl_reg_bit_cg read_en_bit_cg[1];
        kv_read_ctrl_reg_bit_cg read_entry_bit_cg[5];
        kv_read_ctrl_reg_bit_cg pcr_hash_extend_bit_cg[1];
        kv_read_ctrl_reg_bit_cg rsvd_bit_cg[25];
        kv_read_ctrl_reg_fld_cg fld_cg;
        rand dv_base_reg_field read_en;
        rand dv_base_reg_field read_entry;
        rand dv_base_reg_field pcr_hash_extend;
        rand dv_base_reg_field rsvd;

        function new(string name = "kv_read_ctrl_reg");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.read_en = dv_base_reg_field::type_id::create("read_en");
            this.read_en.configure(this, 1, 0, "RW", 1, 'h0, 1, 1, 0);
            this.read_entry = dv_base_reg_field::type_id::create("read_entry");
            this.read_entry.configure(this, 5, 1, "RW", 0, 'h0, 1, 1, 0);
            this.pcr_hash_extend = dv_base_reg_field::type_id::create("pcr_hash_extend");
            this.pcr_hash_extend.configure(this, 1, 6, "RW", 0, 'h0, 1, 1, 0);
            this.rsvd = dv_base_reg_field::type_id::create("rsvd");
            this.rsvd.configure(this, 25, 7, "RW", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(read_en_bit_cg[bt]) read_en_bit_cg[bt] = new();
                foreach(read_entry_bit_cg[bt]) read_entry_bit_cg[bt] = new();
                foreach(pcr_hash_extend_bit_cg[bt]) pcr_hash_extend_bit_cg[bt] = new();
                foreach(rsvd_bit_cg[bt]) rsvd_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : kv_read_ctrl_reg

    // reg - kv_status_reg
    class kv_status_reg extends dv_base_reg;
        `uvm_object_utils(kv_status_reg)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        kv_status_reg_bit_cg READY_bit_cg[1];
        kv_status_reg_bit_cg VALID_bit_cg[1];
        kv_status_reg_bit_cg ERROR_bit_cg[8];
        kv_status_reg_fld_cg fld_cg;
        rand dv_base_reg_field READY;
        rand dv_base_reg_field VALID;
        rand dv_base_reg_field ERROR;

        function new(string name = "kv_status_reg");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.READY = dv_base_reg_field::type_id::create("READY");
            this.READY.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            this.VALID = dv_base_reg_field::type_id::create("VALID");
            this.VALID.configure(this, 1, 1, "RO", 1, 'h0, 1, 1, 0);
            this.ERROR = dv_base_reg_field::type_id::create("ERROR");
            this.ERROR.configure(this, 8, 2, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(READY_bit_cg[bt]) READY_bit_cg[bt] = new();
                foreach(VALID_bit_cg[bt]) VALID_bit_cg[bt] = new();
                foreach(ERROR_bit_cg[bt]) ERROR_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : kv_status_reg

    // reg - kv_write_ctrl_reg
    class kv_write_ctrl_reg extends dv_base_reg;
        `uvm_object_utils(kv_write_ctrl_reg)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        kv_write_ctrl_reg_bit_cg write_en_bit_cg[1];
        kv_write_ctrl_reg_bit_cg write_entry_bit_cg[5];
        kv_write_ctrl_reg_bit_cg hmac_key_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg hmac_block_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg mldsa_seed_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg ecc_pkey_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg ecc_seed_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg aes_key_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg mlkem_seed_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg mlkem_msg_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg dma_data_dest_valid_bit_cg[1];
        kv_write_ctrl_reg_bit_cg rsvd_bit_cg[17];
        kv_write_ctrl_reg_fld_cg fld_cg;
        rand dv_base_reg_field write_en;
        rand dv_base_reg_field write_entry;
        rand dv_base_reg_field hmac_key_dest_valid;
        rand dv_base_reg_field hmac_block_dest_valid;
        rand dv_base_reg_field mldsa_seed_dest_valid;
        rand dv_base_reg_field ecc_pkey_dest_valid;
        rand dv_base_reg_field ecc_seed_dest_valid;
        rand dv_base_reg_field aes_key_dest_valid;
        rand dv_base_reg_field mlkem_seed_dest_valid;
        rand dv_base_reg_field mlkem_msg_dest_valid;
        rand dv_base_reg_field dma_data_dest_valid;
        rand dv_base_reg_field rsvd;

        function new(string name = "kv_write_ctrl_reg");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.write_en = dv_base_reg_field::type_id::create("write_en");
            this.write_en.configure(this, 1, 0, "RW", 1, 'h0, 1, 1, 0);
            this.write_entry = dv_base_reg_field::type_id::create("write_entry");
            this.write_entry.configure(this, 5, 1, "RW", 0, 'h0, 1, 1, 0);
            this.hmac_key_dest_valid = dv_base_reg_field::type_id::create("hmac_key_dest_valid");
            this.hmac_key_dest_valid.configure(this, 1, 6, "RW", 0, 'h0, 1, 1, 0);
            this.hmac_block_dest_valid = dv_base_reg_field::type_id::create("hmac_block_dest_valid");
            this.hmac_block_dest_valid.configure(this, 1, 7, "RW", 0, 'h0, 1, 1, 0);
            this.mldsa_seed_dest_valid = dv_base_reg_field::type_id::create("mldsa_seed_dest_valid");
            this.mldsa_seed_dest_valid.configure(this, 1, 8, "RW", 0, 'h0, 1, 1, 0);
            this.ecc_pkey_dest_valid = dv_base_reg_field::type_id::create("ecc_pkey_dest_valid");
            this.ecc_pkey_dest_valid.configure(this, 1, 9, "RW", 0, 'h0, 1, 1, 0);
            this.ecc_seed_dest_valid = dv_base_reg_field::type_id::create("ecc_seed_dest_valid");
            this.ecc_seed_dest_valid.configure(this, 1, 10, "RW", 0, 'h0, 1, 1, 0);
            this.aes_key_dest_valid = dv_base_reg_field::type_id::create("aes_key_dest_valid");
            this.aes_key_dest_valid.configure(this, 1, 11, "RW", 0, 'h0, 1, 1, 0);
            this.mlkem_seed_dest_valid = dv_base_reg_field::type_id::create("mlkem_seed_dest_valid");
            this.mlkem_seed_dest_valid.configure(this, 1, 12, "RW", 0, 'h0, 1, 1, 0);
            this.mlkem_msg_dest_valid = dv_base_reg_field::type_id::create("mlkem_msg_dest_valid");
            this.mlkem_msg_dest_valid.configure(this, 1, 13, "RW", 0, 'h0, 1, 1, 0);
            this.dma_data_dest_valid = dv_base_reg_field::type_id::create("dma_data_dest_valid");
            this.dma_data_dest_valid.configure(this, 1, 14, "RW", 0, 'h0, 1, 1, 0);
            this.rsvd = dv_base_reg_field::type_id::create("rsvd");
            this.rsvd.configure(this, 17, 15, "RW", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(write_en_bit_cg[bt]) write_en_bit_cg[bt] = new();
                foreach(write_entry_bit_cg[bt]) write_entry_bit_cg[bt] = new();
                foreach(hmac_key_dest_valid_bit_cg[bt]) hmac_key_dest_valid_bit_cg[bt] = new();
                foreach(hmac_block_dest_valid_bit_cg[bt]) hmac_block_dest_valid_bit_cg[bt] = new();
                foreach(mldsa_seed_dest_valid_bit_cg[bt]) mldsa_seed_dest_valid_bit_cg[bt] = new();
                foreach(ecc_pkey_dest_valid_bit_cg[bt]) ecc_pkey_dest_valid_bit_cg[bt] = new();
                foreach(ecc_seed_dest_valid_bit_cg[bt]) ecc_seed_dest_valid_bit_cg[bt] = new();
                foreach(aes_key_dest_valid_bit_cg[bt]) aes_key_dest_valid_bit_cg[bt] = new();
                foreach(mlkem_seed_dest_valid_bit_cg[bt]) mlkem_seed_dest_valid_bit_cg[bt] = new();
                foreach(mlkem_msg_dest_valid_bit_cg[bt]) mlkem_msg_dest_valid_bit_cg[bt] = new();
                foreach(dma_data_dest_valid_bit_cg[bt]) dma_data_dest_valid_bit_cg[bt] = new();
                foreach(rsvd_bit_cg[bt]) rsvd_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : kv_write_ctrl_reg

    // reg - hmac_reg::global_intr_en_t
    class hmac_reg__global_intr_en_t extends dv_base_reg;
        `uvm_object_utils(hmac_reg__global_intr_en_t)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__global_intr_en_t_bit_cg error_en_bit_cg[1];
        hmac_reg__global_intr_en_t_bit_cg notif_en_bit_cg[1];
        hmac_reg__global_intr_en_t_fld_cg fld_cg;
        rand dv_base_reg_field error_en;
        rand dv_base_reg_field notif_en;

        function new(string name = "hmac_reg__global_intr_en_t");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.error_en = dv_base_reg_field::type_id::create("error_en");
            this.error_en.configure(this, 1, 0, "RW", 0, 'h0, 1, 1, 0);
            this.notif_en = dv_base_reg_field::type_id::create("notif_en");
            this.notif_en.configure(this, 1, 1, "RW", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(error_en_bit_cg[bt]) error_en_bit_cg[bt] = new();
                foreach(notif_en_bit_cg[bt]) notif_en_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__global_intr_en_t

    // reg - hmac_reg::error_intr_en_t
    class hmac_reg__error_intr_en_t extends dv_base_reg;
        `uvm_object_utils(hmac_reg__error_intr_en_t)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__error_intr_en_t_bit_cg key_mode_error_en_bit_cg[1];
        hmac_reg__error_intr_en_t_bit_cg key_zero_error_en_bit_cg[1];
        hmac_reg__error_intr_en_t_bit_cg error2_en_bit_cg[1];
        hmac_reg__error_intr_en_t_bit_cg error3_en_bit_cg[1];
        hmac_reg__error_intr_en_t_fld_cg fld_cg;
        rand dv_base_reg_field key_mode_error_en;
        rand dv_base_reg_field key_zero_error_en;
        rand dv_base_reg_field error2_en;
        rand dv_base_reg_field error3_en;

        function new(string name = "hmac_reg__error_intr_en_t");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.key_mode_error_en = dv_base_reg_field::type_id::create("key_mode_error_en");
            this.key_mode_error_en.configure(this, 1, 0, "RW", 0, 'h0, 1, 1, 0);
            this.key_zero_error_en = dv_base_reg_field::type_id::create("key_zero_error_en");
            this.key_zero_error_en.configure(this, 1, 1, "RW", 0, 'h0, 1, 1, 0);
            this.error2_en = dv_base_reg_field::type_id::create("error2_en");
            this.error2_en.configure(this, 1, 2, "RW", 0, 'h0, 1, 1, 0);
            this.error3_en = dv_base_reg_field::type_id::create("error3_en");
            this.error3_en.configure(this, 1, 3, "RW", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(key_mode_error_en_bit_cg[bt]) key_mode_error_en_bit_cg[bt] = new();
                foreach(key_zero_error_en_bit_cg[bt]) key_zero_error_en_bit_cg[bt] = new();
                foreach(error2_en_bit_cg[bt]) error2_en_bit_cg[bt] = new();
                foreach(error3_en_bit_cg[bt]) error3_en_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__error_intr_en_t

    // reg - hmac_reg::notif_intr_en_t
    class hmac_reg__notif_intr_en_t extends dv_base_reg;
        `uvm_object_utils(hmac_reg__notif_intr_en_t)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__notif_intr_en_t_bit_cg notif_cmd_done_en_bit_cg[1];
        hmac_reg__notif_intr_en_t_fld_cg fld_cg;
        rand dv_base_reg_field notif_cmd_done_en;

        function new(string name = "hmac_reg__notif_intr_en_t");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.notif_cmd_done_en = dv_base_reg_field::type_id::create("notif_cmd_done_en");
            this.notif_cmd_done_en.configure(this, 1, 0, "RW", 0, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(notif_cmd_done_en_bit_cg[bt]) notif_cmd_done_en_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__notif_intr_en_t

    // reg - hmac_reg::global_intr_t_agg_sts_dd3dcf0a
    class hmac_reg__global_intr_t_agg_sts_dd3dcf0a extends dv_base_reg;
        `uvm_object_utils(hmac_reg__global_intr_t_agg_sts_dd3dcf0a)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__global_intr_t_agg_sts_dd3dcf0a_bit_cg agg_sts_bit_cg[1];
        hmac_reg__global_intr_t_agg_sts_dd3dcf0a_fld_cg fld_cg;
        rand dv_base_reg_field agg_sts;

        function new(string name = "hmac_reg__global_intr_t_agg_sts_dd3dcf0a");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.agg_sts = dv_base_reg_field::type_id::create("agg_sts");
            this.agg_sts.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(agg_sts_bit_cg[bt]) agg_sts_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__global_intr_t_agg_sts_dd3dcf0a

    // reg - hmac_reg::global_intr_t_agg_sts_e6399b4a
    class hmac_reg__global_intr_t_agg_sts_e6399b4a extends dv_base_reg;
        `uvm_object_utils(hmac_reg__global_intr_t_agg_sts_e6399b4a)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__global_intr_t_agg_sts_e6399b4a_bit_cg agg_sts_bit_cg[1];
        hmac_reg__global_intr_t_agg_sts_e6399b4a_fld_cg fld_cg;
        rand dv_base_reg_field agg_sts;

        function new(string name = "hmac_reg__global_intr_t_agg_sts_e6399b4a");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.agg_sts = dv_base_reg_field::type_id::create("agg_sts");
            this.agg_sts.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(agg_sts_bit_cg[bt]) agg_sts_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__global_intr_t_agg_sts_e6399b4a

    // reg - hmac_reg::error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183
    class hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183_bit_cg key_mode_error_sts_bit_cg[1];
        hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183_bit_cg key_zero_error_sts_bit_cg[1];
        hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183_bit_cg error2_sts_bit_cg[1];
        hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183_bit_cg error3_sts_bit_cg[1];
        hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183_fld_cg fld_cg;
        rand dv_base_reg_field key_mode_error_sts;
        rand dv_base_reg_field key_zero_error_sts;
        rand dv_base_reg_field error2_sts;
        rand dv_base_reg_field error3_sts;

        function new(string name = "hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.key_mode_error_sts = dv_base_reg_field::type_id::create("key_mode_error_sts");
            this.key_mode_error_sts.configure(this, 1, 0, "W1C", 1, 'h0, 1, 1, 0);
            this.key_zero_error_sts = dv_base_reg_field::type_id::create("key_zero_error_sts");
            this.key_zero_error_sts.configure(this, 1, 1, "W1C", 1, 'h0, 1, 1, 0);
            this.error2_sts = dv_base_reg_field::type_id::create("error2_sts");
            this.error2_sts.configure(this, 1, 2, "W1C", 1, 'h0, 1, 1, 0);
            this.error3_sts = dv_base_reg_field::type_id::create("error3_sts");
            this.error3_sts.configure(this, 1, 3, "W1C", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(key_mode_error_sts_bit_cg[bt]) key_mode_error_sts_bit_cg[bt] = new();
                foreach(key_zero_error_sts_bit_cg[bt]) key_zero_error_sts_bit_cg[bt] = new();
                foreach(error2_sts_bit_cg[bt]) error2_sts_bit_cg[bt] = new();
                foreach(error3_sts_bit_cg[bt]) error3_sts_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183

    // reg - hmac_reg::notif_intr_t_notif_cmd_done_sts_1c68637e
    class hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e extends dv_base_reg;
        `uvm_object_utils(hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e_bit_cg notif_cmd_done_sts_bit_cg[1];
        hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e_fld_cg fld_cg;
        rand dv_base_reg_field notif_cmd_done_sts;

        function new(string name = "hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.notif_cmd_done_sts = dv_base_reg_field::type_id::create("notif_cmd_done_sts");
            this.notif_cmd_done_sts.configure(this, 1, 0, "W1C", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(notif_cmd_done_sts_bit_cg[bt]) notif_cmd_done_sts_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e

    // reg - hmac_reg::error_intr_trig_t
    class hmac_reg__error_intr_trig_t extends dv_base_reg;
        `uvm_object_utils(hmac_reg__error_intr_trig_t)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__error_intr_trig_t_bit_cg key_mode_error_trig_bit_cg[1];
        hmac_reg__error_intr_trig_t_bit_cg key_zero_error_trig_bit_cg[1];
        hmac_reg__error_intr_trig_t_bit_cg error2_trig_bit_cg[1];
        hmac_reg__error_intr_trig_t_bit_cg error3_trig_bit_cg[1];
        hmac_reg__error_intr_trig_t_fld_cg fld_cg;
        rand dv_base_reg_field key_mode_error_trig;
        rand dv_base_reg_field key_zero_error_trig;
        rand dv_base_reg_field error2_trig;
        rand dv_base_reg_field error3_trig;

        function new(string name = "hmac_reg__error_intr_trig_t");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.key_mode_error_trig = dv_base_reg_field::type_id::create("key_mode_error_trig");
            this.key_mode_error_trig.configure(this, 1, 0, "W1S", 1, 'h0, 1, 1, 0);
            this.key_zero_error_trig = dv_base_reg_field::type_id::create("key_zero_error_trig");
            this.key_zero_error_trig.configure(this, 1, 1, "W1S", 1, 'h0, 1, 1, 0);
            this.error2_trig = dv_base_reg_field::type_id::create("error2_trig");
            this.error2_trig.configure(this, 1, 2, "W1S", 1, 'h0, 1, 1, 0);
            this.error3_trig = dv_base_reg_field::type_id::create("error3_trig");
            this.error3_trig.configure(this, 1, 3, "W1S", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(key_mode_error_trig_bit_cg[bt]) key_mode_error_trig_bit_cg[bt] = new();
                foreach(key_zero_error_trig_bit_cg[bt]) key_zero_error_trig_bit_cg[bt] = new();
                foreach(error2_trig_bit_cg[bt]) error2_trig_bit_cg[bt] = new();
                foreach(error3_trig_bit_cg[bt]) error3_trig_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__error_intr_trig_t

    // reg - hmac_reg::notif_intr_trig_t
    class hmac_reg__notif_intr_trig_t extends dv_base_reg;
        `uvm_object_utils(hmac_reg__notif_intr_trig_t)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__notif_intr_trig_t_bit_cg notif_cmd_done_trig_bit_cg[1];
        hmac_reg__notif_intr_trig_t_fld_cg fld_cg;
        rand dv_base_reg_field notif_cmd_done_trig;

        function new(string name = "hmac_reg__notif_intr_trig_t");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.notif_cmd_done_trig = dv_base_reg_field::type_id::create("notif_cmd_done_trig");
            this.notif_cmd_done_trig.configure(this, 1, 0, "W1S", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(notif_cmd_done_trig_bit_cg[bt]) notif_cmd_done_trig_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__notif_intr_trig_t

    // reg - hmac_reg::intr_count_t_cnt_b8d41777
    class hmac_reg__intr_count_t_cnt_b8d41777 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_t_cnt_b8d41777)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_t_cnt_b8d41777_bit_cg cnt_bit_cg[32];
        hmac_reg__intr_count_t_cnt_b8d41777_fld_cg fld_cg;
        rand dv_base_reg_field cnt;

        function new(string name = "hmac_reg__intr_count_t_cnt_b8d41777");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.cnt = dv_base_reg_field::type_id::create("cnt");
            this.cnt.configure(this, 32, 0, "RW", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(cnt_bit_cg[bt]) cnt_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_t_cnt_b8d41777

    // reg - hmac_reg::intr_count_t_cnt_55fc66d7
    class hmac_reg__intr_count_t_cnt_55fc66d7 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_t_cnt_55fc66d7)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_t_cnt_55fc66d7_bit_cg cnt_bit_cg[32];
        hmac_reg__intr_count_t_cnt_55fc66d7_fld_cg fld_cg;
        rand dv_base_reg_field cnt;

        function new(string name = "hmac_reg__intr_count_t_cnt_55fc66d7");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.cnt = dv_base_reg_field::type_id::create("cnt");
            this.cnt.configure(this, 32, 0, "RW", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(cnt_bit_cg[bt]) cnt_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_t_cnt_55fc66d7

    // reg - hmac_reg::intr_count_t_cnt_d8af96ff
    class hmac_reg__intr_count_t_cnt_d8af96ff extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_t_cnt_d8af96ff)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_t_cnt_d8af96ff_bit_cg cnt_bit_cg[32];
        hmac_reg__intr_count_t_cnt_d8af96ff_fld_cg fld_cg;
        rand dv_base_reg_field cnt;

        function new(string name = "hmac_reg__intr_count_t_cnt_d8af96ff");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.cnt = dv_base_reg_field::type_id::create("cnt");
            this.cnt.configure(this, 32, 0, "RW", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(cnt_bit_cg[bt]) cnt_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_t_cnt_d8af96ff

    // reg - hmac_reg::intr_count_t_cnt_9bd7f809
    class hmac_reg__intr_count_t_cnt_9bd7f809 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_t_cnt_9bd7f809)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_t_cnt_9bd7f809_bit_cg cnt_bit_cg[32];
        hmac_reg__intr_count_t_cnt_9bd7f809_fld_cg fld_cg;
        rand dv_base_reg_field cnt;

        function new(string name = "hmac_reg__intr_count_t_cnt_9bd7f809");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.cnt = dv_base_reg_field::type_id::create("cnt");
            this.cnt.configure(this, 32, 0, "RW", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(cnt_bit_cg[bt]) cnt_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_t_cnt_9bd7f809

    // reg - hmac_reg::intr_count_t_cnt_be67d6d5
    class hmac_reg__intr_count_t_cnt_be67d6d5 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_t_cnt_be67d6d5)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_t_cnt_be67d6d5_bit_cg cnt_bit_cg[32];
        hmac_reg__intr_count_t_cnt_be67d6d5_fld_cg fld_cg;
        rand dv_base_reg_field cnt;

        function new(string name = "hmac_reg__intr_count_t_cnt_be67d6d5");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.cnt = dv_base_reg_field::type_id::create("cnt");
            this.cnt.configure(this, 32, 0, "RW", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(cnt_bit_cg[bt]) cnt_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_t_cnt_be67d6d5

    // reg - hmac_reg::intr_count_incr_t_pulse_14f62453
    class hmac_reg__intr_count_incr_t_pulse_14f62453 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_incr_t_pulse_14f62453)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_incr_t_pulse_14f62453_bit_cg pulse_bit_cg[1];
        hmac_reg__intr_count_incr_t_pulse_14f62453_fld_cg fld_cg;
        rand dv_base_reg_field pulse;

        function new(string name = "hmac_reg__intr_count_incr_t_pulse_14f62453");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.pulse = dv_base_reg_field::type_id::create("pulse");
            this.pulse.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(pulse_bit_cg[bt]) pulse_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_incr_t_pulse_14f62453

    // reg - hmac_reg::intr_count_incr_t_pulse_080329b7
    class hmac_reg__intr_count_incr_t_pulse_080329b7 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_incr_t_pulse_080329b7)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_incr_t_pulse_080329b7_bit_cg pulse_bit_cg[1];
        hmac_reg__intr_count_incr_t_pulse_080329b7_fld_cg fld_cg;
        rand dv_base_reg_field pulse;

        function new(string name = "hmac_reg__intr_count_incr_t_pulse_080329b7");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.pulse = dv_base_reg_field::type_id::create("pulse");
            this.pulse.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(pulse_bit_cg[bt]) pulse_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_incr_t_pulse_080329b7

    // reg - hmac_reg::intr_count_incr_t_pulse_87b45fe7
    class hmac_reg__intr_count_incr_t_pulse_87b45fe7 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_incr_t_pulse_87b45fe7)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_incr_t_pulse_87b45fe7_bit_cg pulse_bit_cg[1];
        hmac_reg__intr_count_incr_t_pulse_87b45fe7_fld_cg fld_cg;
        rand dv_base_reg_field pulse;

        function new(string name = "hmac_reg__intr_count_incr_t_pulse_87b45fe7");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.pulse = dv_base_reg_field::type_id::create("pulse");
            this.pulse.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(pulse_bit_cg[bt]) pulse_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_incr_t_pulse_87b45fe7

    // reg - hmac_reg::intr_count_incr_t_pulse_c1689ee6
    class hmac_reg__intr_count_incr_t_pulse_c1689ee6 extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_incr_t_pulse_c1689ee6)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_incr_t_pulse_c1689ee6_bit_cg pulse_bit_cg[1];
        hmac_reg__intr_count_incr_t_pulse_c1689ee6_fld_cg fld_cg;
        rand dv_base_reg_field pulse;

        function new(string name = "hmac_reg__intr_count_incr_t_pulse_c1689ee6");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.pulse = dv_base_reg_field::type_id::create("pulse");
            this.pulse.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(pulse_bit_cg[bt]) pulse_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_incr_t_pulse_c1689ee6

    // reg - hmac_reg::intr_count_incr_t_pulse_6173128e
    class hmac_reg__intr_count_incr_t_pulse_6173128e extends dv_base_reg;
        `uvm_object_utils(hmac_reg__intr_count_incr_t_pulse_6173128e)
        protected uvm_reg_data_t m_current;
        protected uvm_reg_data_t m_data;
        protected bit            m_is_read;

        hmac_reg__intr_count_incr_t_pulse_6173128e_bit_cg pulse_bit_cg[1];
        hmac_reg__intr_count_incr_t_pulse_6173128e_fld_cg fld_cg;
        rand dv_base_reg_field pulse;

        function new(string name = "hmac_reg__intr_count_incr_t_pulse_6173128e");
            super.new(name, 32, build_coverage(UVM_CVR_ALL));
        endfunction : new
        extern virtual function void sample_values();
        extern protected virtual function void sample(uvm_reg_data_t  data,
                                                      uvm_reg_data_t  byte_en,
                                                      bit             is_read,
                                                      uvm_reg_map     map);

        virtual function void build();
            this.pulse = dv_base_reg_field::type_id::create("pulse");
            this.pulse.configure(this, 1, 0, "RO", 1, 'h0, 1, 1, 0);
            if (has_coverage(UVM_CVR_REG_BITS)) begin
                foreach(pulse_bit_cg[bt]) pulse_bit_cg[bt] = new();
            end
            if (has_coverage(UVM_CVR_FIELD_VALS))
                fld_cg = new();
        endfunction : build
    endclass : hmac_reg__intr_count_incr_t_pulse_6173128e

    // regfile - hmac_reg::intr_block_t
    class hmac_reg__intr_block_t extends dv_base_reg_block;
        `uvm_object_utils(hmac_reg__intr_block_t)
        rand hmac_reg__global_intr_en_t global_intr_en_r;
        rand hmac_reg__error_intr_en_t error_intr_en_r;
        rand hmac_reg__notif_intr_en_t notif_intr_en_r;
        rand hmac_reg__global_intr_t_agg_sts_dd3dcf0a error_global_intr_r;
        rand hmac_reg__global_intr_t_agg_sts_e6399b4a notif_global_intr_r;
        rand hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183 error_internal_intr_r;
        rand hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e notif_internal_intr_r;
        rand hmac_reg__error_intr_trig_t error_intr_trig_r;
        rand hmac_reg__notif_intr_trig_t notif_intr_trig_r;
        rand hmac_reg__intr_count_t_cnt_b8d41777 key_mode_error_intr_count_r;
        rand hmac_reg__intr_count_t_cnt_55fc66d7 key_zero_error_intr_count_r;
        rand hmac_reg__intr_count_t_cnt_d8af96ff error2_intr_count_r;
        rand hmac_reg__intr_count_t_cnt_9bd7f809 error3_intr_count_r;
        rand hmac_reg__intr_count_t_cnt_be67d6d5 notif_cmd_done_intr_count_r;
        rand hmac_reg__intr_count_incr_t_pulse_14f62453 key_mode_error_intr_count_incr_r;
        rand hmac_reg__intr_count_incr_t_pulse_080329b7 key_zero_error_intr_count_incr_r;
        rand hmac_reg__intr_count_incr_t_pulse_87b45fe7 error2_intr_count_incr_r;
        rand hmac_reg__intr_count_incr_t_pulse_c1689ee6 error3_intr_count_incr_r;
        rand hmac_reg__intr_count_incr_t_pulse_6173128e notif_cmd_done_intr_count_incr_r;

        function new(string name = "hmac_reg__intr_block_t");
            super.new(name);
        endfunction : new

        virtual function void build();
            this.default_map = create_map("reg_map", 0, 4, UVM_NO_ENDIAN);
            this.global_intr_en_r = hmac_reg__global_intr_en_t::type_id::create("global_intr_en_r");
            this.global_intr_en_r.configure(this);

            this.global_intr_en_r.build();
            this.default_map.add_reg(this.global_intr_en_r, 'h0);
            this.error_intr_en_r = hmac_reg__error_intr_en_t::type_id::create("error_intr_en_r");
            this.error_intr_en_r.configure(this);

            this.error_intr_en_r.build();
            this.default_map.add_reg(this.error_intr_en_r, 'h4);
            this.notif_intr_en_r = hmac_reg__notif_intr_en_t::type_id::create("notif_intr_en_r");
            this.notif_intr_en_r.configure(this);

            this.notif_intr_en_r.build();
            this.default_map.add_reg(this.notif_intr_en_r, 'h8);
            this.error_global_intr_r = hmac_reg__global_intr_t_agg_sts_dd3dcf0a::type_id::create("error_global_intr_r");
            this.error_global_intr_r.configure(this);

            this.error_global_intr_r.build();
            this.default_map.add_reg(this.error_global_intr_r, 'hc);
            this.notif_global_intr_r = hmac_reg__global_intr_t_agg_sts_e6399b4a::type_id::create("notif_global_intr_r");
            this.notif_global_intr_r.configure(this);

            this.notif_global_intr_r.build();
            this.default_map.add_reg(this.notif_global_intr_r, 'h10);
            this.error_internal_intr_r = hmac_reg__error_intr_t_error2_sts_b1cf2205_error3_sts_74a35378_key_mode_error_sts_f2304c86_key_zero_error_sts_64a18183::type_id::create("error_internal_intr_r");
            this.error_internal_intr_r.configure(this);

            this.error_internal_intr_r.build();
            this.default_map.add_reg(this.error_internal_intr_r, 'h14);
            this.notif_internal_intr_r = hmac_reg__notif_intr_t_notif_cmd_done_sts_1c68637e::type_id::create("notif_internal_intr_r");
            this.notif_internal_intr_r.configure(this);

            this.notif_internal_intr_r.build();
            this.default_map.add_reg(this.notif_internal_intr_r, 'h18);
            this.error_intr_trig_r = hmac_reg__error_intr_trig_t::type_id::create("error_intr_trig_r");
            this.error_intr_trig_r.configure(this);

            this.error_intr_trig_r.build();
            this.default_map.add_reg(this.error_intr_trig_r, 'h1c);
            this.notif_intr_trig_r = hmac_reg__notif_intr_trig_t::type_id::create("notif_intr_trig_r");
            this.notif_intr_trig_r.configure(this);

            this.notif_intr_trig_r.build();
            this.default_map.add_reg(this.notif_intr_trig_r, 'h20);
            this.key_mode_error_intr_count_r = hmac_reg__intr_count_t_cnt_b8d41777::type_id::create("key_mode_error_intr_count_r");
            this.key_mode_error_intr_count_r.configure(this);

            this.key_mode_error_intr_count_r.build();
            this.default_map.add_reg(this.key_mode_error_intr_count_r, 'h100);
            this.key_zero_error_intr_count_r = hmac_reg__intr_count_t_cnt_55fc66d7::type_id::create("key_zero_error_intr_count_r");
            this.key_zero_error_intr_count_r.configure(this);

            this.key_zero_error_intr_count_r.build();
            this.default_map.add_reg(this.key_zero_error_intr_count_r, 'h104);
            this.error2_intr_count_r = hmac_reg__intr_count_t_cnt_d8af96ff::type_id::create("error2_intr_count_r");
            this.error2_intr_count_r.configure(this);

            this.error2_intr_count_r.build();
            this.default_map.add_reg(this.error2_intr_count_r, 'h108);
            this.error3_intr_count_r = hmac_reg__intr_count_t_cnt_9bd7f809::type_id::create("error3_intr_count_r");
            this.error3_intr_count_r.configure(this);

            this.error3_intr_count_r.build();
            this.default_map.add_reg(this.error3_intr_count_r, 'h10c);
            this.notif_cmd_done_intr_count_r = hmac_reg__intr_count_t_cnt_be67d6d5::type_id::create("notif_cmd_done_intr_count_r");
            this.notif_cmd_done_intr_count_r.configure(this);

            this.notif_cmd_done_intr_count_r.build();
            this.default_map.add_reg(this.notif_cmd_done_intr_count_r, 'h180);
            this.key_mode_error_intr_count_incr_r = hmac_reg__intr_count_incr_t_pulse_14f62453::type_id::create("key_mode_error_intr_count_incr_r");
            this.key_mode_error_intr_count_incr_r.configure(this);

            this.key_mode_error_intr_count_incr_r.build();
            this.default_map.add_reg(this.key_mode_error_intr_count_incr_r, 'h200);
            this.key_zero_error_intr_count_incr_r = hmac_reg__intr_count_incr_t_pulse_080329b7::type_id::create("key_zero_error_intr_count_incr_r");
            this.key_zero_error_intr_count_incr_r.configure(this);

            this.key_zero_error_intr_count_incr_r.build();
            this.default_map.add_reg(this.key_zero_error_intr_count_incr_r, 'h204);
            this.error2_intr_count_incr_r = hmac_reg__intr_count_incr_t_pulse_87b45fe7::type_id::create("error2_intr_count_incr_r");
            this.error2_intr_count_incr_r.configure(this);

            this.error2_intr_count_incr_r.build();
            this.default_map.add_reg(this.error2_intr_count_incr_r, 'h208);
            this.error3_intr_count_incr_r = hmac_reg__intr_count_incr_t_pulse_c1689ee6::type_id::create("error3_intr_count_incr_r");
            this.error3_intr_count_incr_r.configure(this);

            this.error3_intr_count_incr_r.build();
            this.default_map.add_reg(this.error3_intr_count_incr_r, 'h20c);
            this.notif_cmd_done_intr_count_incr_r = hmac_reg__intr_count_incr_t_pulse_6173128e::type_id::create("notif_cmd_done_intr_count_incr_r");
            this.notif_cmd_done_intr_count_incr_r.configure(this);

            this.notif_cmd_done_intr_count_incr_r.build();
            this.default_map.add_reg(this.notif_cmd_done_intr_count_incr_r, 'h210);
        endfunction : build
    endclass : hmac_reg__intr_block_t

    // addrmap - hmac_reg
    class hmac_reg extends dv_base_reg_block;
        `uvm_object_utils(hmac_reg)
        rand hmac_reg__HMAC512_NAME HMAC512_NAME[2];
        rand hmac_reg__HMAC512_VERSION HMAC512_VERSION[2];
        rand hmac_reg__HMAC512_CTRL HMAC512_CTRL;
        rand hmac_reg__HMAC512_STATUS HMAC512_STATUS;
        rand hmac_reg__HMAC512_KEY HMAC512_KEY[16];
        rand hmac_reg__HMAC512_BLOCK HMAC512_BLOCK[32];
        rand hmac_reg__HMAC512_TAG HMAC512_TAG[16];
        rand hmac_reg__HMAC512_LFSR_SEED HMAC512_LFSR_SEED[6];
        rand kv_read_ctrl_reg HMAC512_KV_RD_KEY_CTRL;
        rand kv_status_reg HMAC512_KV_RD_KEY_STATUS;
        rand kv_read_ctrl_reg HMAC512_KV_RD_BLOCK_CTRL;
        rand kv_status_reg HMAC512_KV_RD_BLOCK_STATUS;
        rand kv_write_ctrl_reg HMAC512_KV_WR_CTRL;
        rand kv_status_reg HMAC512_KV_WR_STATUS;
        rand hmac_reg__intr_block_t intr_block_rf;

        function new(string name = "hmac_reg");
            super.new(name);
        endfunction : new

        virtual function void build();
            this.default_map = create_map("reg_map", 0, 4, UVM_NO_ENDIAN);
            foreach(this.HMAC512_NAME[i0]) begin
                this.HMAC512_NAME[i0] = hmac_reg__HMAC512_NAME::type_id::create($sformatf("HMAC512_NAME[%0d]", i0));
                this.HMAC512_NAME[i0].configure(this);

                this.HMAC512_NAME[i0].build();
                this.default_map.add_reg(this.HMAC512_NAME[i0], 'h0 + i0*'h4);
            end
            foreach(this.HMAC512_VERSION[i0]) begin
                this.HMAC512_VERSION[i0] = hmac_reg__HMAC512_VERSION::type_id::create($sformatf("HMAC512_VERSION[%0d]", i0));
                this.HMAC512_VERSION[i0].configure(this);

                this.HMAC512_VERSION[i0].build();
                this.default_map.add_reg(this.HMAC512_VERSION[i0], 'h8 + i0*'h4);
            end
            this.HMAC512_CTRL = hmac_reg__HMAC512_CTRL::type_id::create("HMAC512_CTRL");
            this.HMAC512_CTRL.configure(this);

            this.HMAC512_CTRL.build();
            this.default_map.add_reg(this.HMAC512_CTRL, 'h10);
            this.HMAC512_STATUS = hmac_reg__HMAC512_STATUS::type_id::create("HMAC512_STATUS");
            this.HMAC512_STATUS.configure(this);

            this.HMAC512_STATUS.build();
            this.default_map.add_reg(this.HMAC512_STATUS, 'h18);
            foreach(this.HMAC512_KEY[i0]) begin
                this.HMAC512_KEY[i0] = hmac_reg__HMAC512_KEY::type_id::create($sformatf("HMAC512_KEY[%0d]", i0));
                this.HMAC512_KEY[i0].configure(this);

                this.HMAC512_KEY[i0].build();
                this.default_map.add_reg(this.HMAC512_KEY[i0], 'h40 + i0*'h4);
            end
            foreach(this.HMAC512_BLOCK[i0]) begin
                this.HMAC512_BLOCK[i0] = hmac_reg__HMAC512_BLOCK::type_id::create($sformatf("HMAC512_BLOCK[%0d]", i0));
                this.HMAC512_BLOCK[i0].configure(this);

                this.HMAC512_BLOCK[i0].build();
                this.default_map.add_reg(this.HMAC512_BLOCK[i0], 'h80 + i0*'h4);
            end
            foreach(this.HMAC512_TAG[i0]) begin
                this.HMAC512_TAG[i0] = hmac_reg__HMAC512_TAG::type_id::create($sformatf("HMAC512_TAG[%0d]", i0));
                this.HMAC512_TAG[i0].configure(this);

                this.HMAC512_TAG[i0].build();
                this.default_map.add_reg(this.HMAC512_TAG[i0], 'h100 + i0*'h4);
            end
            foreach(this.HMAC512_LFSR_SEED[i0]) begin
                this.HMAC512_LFSR_SEED[i0] = hmac_reg__HMAC512_LFSR_SEED::type_id::create($sformatf("HMAC512_LFSR_SEED[%0d]", i0));
                this.HMAC512_LFSR_SEED[i0].configure(this);

                this.HMAC512_LFSR_SEED[i0].build();
                this.default_map.add_reg(this.HMAC512_LFSR_SEED[i0], 'h140 + i0*'h4);
            end
            this.HMAC512_KV_RD_KEY_CTRL = kv_read_ctrl_reg::type_id::create("HMAC512_KV_RD_KEY_CTRL");
            this.HMAC512_KV_RD_KEY_CTRL.configure(this);

            this.HMAC512_KV_RD_KEY_CTRL.build();
            this.default_map.add_reg(this.HMAC512_KV_RD_KEY_CTRL, 'h600);
            this.HMAC512_KV_RD_KEY_STATUS = kv_status_reg::type_id::create("HMAC512_KV_RD_KEY_STATUS");
            this.HMAC512_KV_RD_KEY_STATUS.configure(this);

            this.HMAC512_KV_RD_KEY_STATUS.build();
            this.default_map.add_reg(this.HMAC512_KV_RD_KEY_STATUS, 'h604);
            this.HMAC512_KV_RD_BLOCK_CTRL = kv_read_ctrl_reg::type_id::create("HMAC512_KV_RD_BLOCK_CTRL");
            this.HMAC512_KV_RD_BLOCK_CTRL.configure(this);

            this.HMAC512_KV_RD_BLOCK_CTRL.build();
            this.default_map.add_reg(this.HMAC512_KV_RD_BLOCK_CTRL, 'h608);
            this.HMAC512_KV_RD_BLOCK_STATUS = kv_status_reg::type_id::create("HMAC512_KV_RD_BLOCK_STATUS");
            this.HMAC512_KV_RD_BLOCK_STATUS.configure(this);

            this.HMAC512_KV_RD_BLOCK_STATUS.build();
            this.default_map.add_reg(this.HMAC512_KV_RD_BLOCK_STATUS, 'h60c);
            this.HMAC512_KV_WR_CTRL = kv_write_ctrl_reg::type_id::create("HMAC512_KV_WR_CTRL");
            this.HMAC512_KV_WR_CTRL.configure(this);

            this.HMAC512_KV_WR_CTRL.build();
            this.default_map.add_reg(this.HMAC512_KV_WR_CTRL, 'h610);
            this.HMAC512_KV_WR_STATUS = kv_status_reg::type_id::create("HMAC512_KV_WR_STATUS");
            this.HMAC512_KV_WR_STATUS.configure(this);

            this.HMAC512_KV_WR_STATUS.build();
            this.default_map.add_reg(this.HMAC512_KV_WR_STATUS, 'h614);
            this.intr_block_rf = hmac_reg__intr_block_t::type_id::create("intr_block_rf");
            this.intr_block_rf.configure(this);
            this.intr_block_rf.build();
            this.default_map.add_submap(this.intr_block_rf.default_map, 'h800);
        endfunction : build
    endclass : hmac_reg

    `include "hmac_reg_sample.svh"
endpackage: hmac_reg_uvm
