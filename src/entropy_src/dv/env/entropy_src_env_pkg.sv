...
package entropy_src_env_pkg;
  ...
  // An import that makes sure we pull in the register model package. Accesses are actually done
  // with explicit the package name (because the register block is called just "entropy_src"), so
  // this import is just pulling in some other arbitrary name from the package.
  import entropy_src_uvm::entropy_src__INTERRUPT_STATE;
  ...
  // parameters
  parameter bit [31:0] INCR_ENTROPY_LO    = 32'h76543210;
  parameter bit [31:0] INCR_ENTROPY_HI    = 32'hfedcba98;
