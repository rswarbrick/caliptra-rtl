...
class entropy_src_err_vseq extends entropy_src_base_vseq;
  ...
  task body();
    ...
    csr_wr(.ptr(ral.INTERRUPT_ENABLE.ES_FATAL_ERR), .value(1'b1), .predict(1'b1));
    ...
    cfg.m_assertion_vif.control_assertions(0);
  ...
  task post_start();
    // Re-enable any assertions that were disabled earlier in the sequence
    cfg.m_assertion_vif.control_assertions(1);
  endtask
