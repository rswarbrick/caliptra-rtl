...
class entropy_src_intr_vseq extends entropy_src_base_vseq;
  ...
  task test_es_entropy_valid();
    ...
    csr_wr(.ptr(ral.INTERRUPT_ENABLE), .value(1 << EntropyValid));
    // Turn on the DUT.
    enable_dut();
    ...
    csr_spinwait(.ptr(ral.INTERRUPT_STATE.ES_ENTROPY_VALID), .exp_data(1'b1));
  ...
  task test_es_observe_fifo_ready();
    ...
    csr_wr(.ptr(ral.INTERRUPT_ENABLE), .value(1 << ObserveFifoReady));
    // Turn on the DUT.
    enable_dut();
    ...
    csr_spinwait(.ptr(ral.INTERRUPT_STATE.ES_OBSERVE_FIFO_READY), .exp_data(1'b1));
  ...
  task test_es_health_test_failed();
    ...
    csr_wr(.ptr(ral.INTERRUPT_ENABLE), .value(1 << HealthTestFailed));
    ...
    csr_spinwait(.ptr(ral.INTERRUPT_STATE.ES_HEALTH_TEST_FAILED), .exp_data(1'b1));
