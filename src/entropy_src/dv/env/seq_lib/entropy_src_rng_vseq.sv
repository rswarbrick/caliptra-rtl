...
class entropy_src_rng_vseq extends entropy_src_base_vseq;
  ...
  task process_interrupts();
    ...
    csr_rd(.ptr(ral.INTERRUPT_STATE), .value(intr_status));
  ...
  task check_reconfig();
    uvm_reg lockable_conf_regs[] = '{ral.CONF, ral.ENTROPY_CONTROL, ral.HEALTH_TEST_WINDOWS,
                                     ral.REPCNT_THRESHOLDS, ral.REPCNTS_THRESHOLDS,
                                     ral.ADAPTP_HI_THRESHOLDS, ral.ADAPTP_LO_THRESHOLDS,
                                     ral.BUCKET_THRESHOLDS, ral.FW_OV_CONTROL,
                                     ral.MARKOV_HI_THRESHOLDS, ral.MARKOV_LO_THRESHOLDS,
                                     ral.OBSERVE_FIFO_THRESH, ral.ALERT_THRESHOLD};
    foreach (lockable_conf_regs[i]) begin
      uvm_status_e txn_status;
      lockable_conf_regs[i].mirror(txn_status);

      if (cfg.under_reset) return;
      if (txn_status != UVM_IS_OK) begin
        `uvm_error(get_full_name(),
                   $sformatf("Failed to mirror %0s", lockable_conf_regs[i].get_name()))
      end
    end
  endtask
  ...
  task entropy_inject_thread();
    ...
    if (do_inject) begin
      while (do_background_procs) begin
        ...
        ral.FW_OV_SHA3_START.update(txn_status);
        if (cfg.under_reset) return;
        if (txn_status != UVM_IS_OK) begin
          `uvm_error(get_full_name(), "Failed to update fw_ov_sha3_start.")
        end
        ...
        for (int i = 0; i < fw_ov_insert_per_seed; i++) begin
          ...
          cfg.clk_rst_vif.wait_clks_or_rst(dly_to_insert_entropy);
          if (cfg.under_reset) return;

          `uvm_info(`gfn, $sformatf("injecting entropy: %08x", fw_ov_value), UVM_FULL)

          ral.FW_OV_WR_FIFO_FULL.mirror(txn_status);
          if (cfg.under_reset) return;
          if (txn_status != UVM_IS_OK) begin
            `uvm_error(get_full_name(), "Failed to mirror fw_ov_wr_fifo_full.")
          end

          ral.FW_OV_WR_DATA.set(fw_ov_value);
          ral.FW_OV_WR_DATA.update(txn_status);
          if (cfg.under_reset) return;
          if (txn_status != UVM_IS_OK) begin
            `uvm_error(get_full_name(), "Failed to update fw_ov_wr_data.")
          end
        end
        ...
        if (cfg.fsm_tracking_vif.next_state == rare_fsm_states[i]) begin
          ...
          if (!rare_state_to_idle_checklist[i]) begin
            ...
            if (cfg.induce_targeted_transition) begin
              ...
              ral.MODULE_ENABLE.write(txn_status, MuBi4False);
              if (cfg.under_reset) return;
              if (txn_status != UVM_IS_OK) begin
                `uvm_error(get_full_name(), "Failed to write MODULE_ENABLE.")
              end
