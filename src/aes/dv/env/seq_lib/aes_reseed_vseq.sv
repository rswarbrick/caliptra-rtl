...
class aes_reseed_vseq extends aes_base_vseq;
    ...
    fork : isolation_fork begin
      fork
        ...
        begin
          ...
          `uvm_error(get_name(), "Timeout waiting for entropy_clearing_req")
        end
      join_any
      disable fork;
    end join
  ...
  local task check_masking_prng_reseed();
    ...
    fork : isolation_fork_rise begin
      fork
        ...
        begin
          ...
          `uvm_error(get_name(), "Timeout waiting for entropy_masking_req to go high")
        end
      join_any
      disable fork;
    end join
    ...
    `uvm_info(get_name(), "Waiting for entropy_masking_req to go low again", UVM_MEDIUM)
    fork : isolation_fork_fall begin
      fork
        ...
        begin
          cfg.clk_rst_vif.wait_clks(wait_timeout_cycles_max);
          `uvm_error(get_name(), "Timeout waiting for entropy_masking_req to go low again")
        end
      join_any
      disable fork;
    end join
